...
export function Board({ tasks, now, activity, activityKind, context, models, projects, onOpen, onAddTask }: Props) {
  const cols = groupColumns(tasks);
  const failed = tasks.filter((t) => t.status === "failed");
  const running = [...failed, ...cols.running];

  const shared = { now, activity, activityKind, context, models, projects, onOpen };
